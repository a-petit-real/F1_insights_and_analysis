...
- **Dépassements** (`getOvertakes`) — la table `overtakes` est ingérée telle quelle depuis OpenF1, mais son flux contient parfois du bruit : pour deux voitures quasi à égalité, l'API rapporte une paire réciproque en moins d'une seconde (A dépasse B, puis B dépasse A) — physiquement impossible pour un vrai dépassement suivi d'un contre (constaté sur les données réelles Monza : écarts de 0.036s à 0.179s entre les deux lignes d'une même paire). Filtrée côté web (`dedupeOvertakeFlaps`, seuil 500ms) : toute paire réciproque sous ce seuil est retirée des deux côtés, aucune des deux directions n'étant vérifiable comme "la vraie". Les échanges de position réels lors d'une bagarre serrée (plusieurs secondes d'écart, ex. 3.318s observé) restent affichés.
